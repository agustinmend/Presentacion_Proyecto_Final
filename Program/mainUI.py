...
def attempt_connection():
    ...
    sounds.click()
    from main import connection_to_db  # Import connection function
...
def action_table(selected_table):
    ...
    if selected_table['first'] == "visualize":
        ...
        modify_window.title(f"Add data to the {selected_table['second']} table")
        ...
        for i, column in enumerate(columns):
            ...
            reg_entry = tk.Entry(modify_window)
            reg_entry.grid(row=i + 1, column=1, padx=10, pady=5, sticky="ew")
            entries[column] = reg_entry  # Save entry reference with column name as key
        ...
        # Function to handle data insertion
        def handle_update():
            selected_item = treeview.selection()  # Asegúrate de que treeview está definido y contiene datos
            if not selected_item:
                tk.messagebox.showerror("Error", "No row selected!")  # Validación si no se seleccionó ninguna fila
                return

            # Obtener el id de la fila seleccionada (suponiendo que la columna 'id' es la primera columna)
            try:
                selected_row_id = treeview.item(selected_item, 'values')[0]  # Ajusta el índice si es necesario
            except IndexError:
                tk.messagebox.showerror("Error", "Invalid row selected!")
                return

            # Ahora puedes usar selected_row_id en la condición de la consulta
            values = {column: entry.get() for column, entry in entries.items()}
            condition = f"id = {selected_row_id}"

            success = main.update_data(main.con, selected_table['second'], values, condition)
            if success:
                sounds.succed_task()
                tk.messagebox.showinfo("Success", "Data updated successfully.")
                modify_window.destroy()  # Corregido el nombre de la ventana a modify_window
            else:
                tk.messagebox.showerror("Error", "Failed to update data.")
        # Submit button to add data
